...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      setIsScrolled(window.scrollY > 80); // Lower threshold for mobile feel
    };
  ...
  const handleLoginSuccess = () => {
    ...
    // Removed the automatic opening of the profile modal here
    // to prevent jarring navigation for the user after onboarding.
  };
    ...
    <div className="w-full min-h-screen bg-[#0c0c0c] text-white pb-48 animate-in fade-in duration-700 overflow-x-hidden relative">
      ...
      <div className={`sticky top-0 z-50 bg-[#0c0c0c]/98 backdrop-blur-xl transition-all duration-300 ${isScrolled ? 'border-b border-zinc-900 shadow-xl' : ''}`}>
        <div className="max-w-[430px] mx-auto">
          {/* Logo Row */}
          <header className="px-5 pt-[env(safe-area-inset-top,40px)] pb-2 flex justify-between items-center">
            <div className="flex items-center gap-2">
              <span className="text-xl sm:text-[22px] font-bold tracking-tight text-white">Boketto</span>
          ...
          <div className="px-5 flex gap-3 items-center mt-3 mb-4">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full bg-[#1a1a1a] border border-zinc-800 rounded-2xl py-3 pr-4 pl-11 text-sm font-medium focus:outline-none focus:ring-1 focus:ring-zinc-700"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
              />
            </div>
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-black text-zinc-400 tracking-tighter uppercase">Veg</span>
              <button 
                onClick={() => setIsVegOnly(!isVegOnly)}
                className={`w-[40px] h-[20px] rounded-full relative transition-all duration-300 ${isVegOnly ? 'bg-[#22c55e]' : 'bg-zinc-800'}`}
              >
                <div className={`absolute top-[2px] w-[16px] h-[16px] bg-white rounded-full transition-all shadow-sm ${isVegOnly ? 'left-[22px]' : 'left-[2px]'}`} />
              </button>
            </div>
          </div>

          {/* Sticky Compact Elements - Shows on Scroll */}
          {isScrolled && (
            <div className="animate-in slide-in-from-top-1 duration-300 px-5 pb-2">
              <CompactGoalBar />
      ...
      {/* Main Content Area */}
      <div className="max-w-[430px] mx-auto mt-4">
         {/* Original Large Elements - Hidden on Scroll */}
         <div className={`transition-all duration-300 ${isScrolled ? 'opacity-0 pointer-events-none -mt-32' : 'opacity-100'}`}>
        ...
        {/* Section Heading */}
        <div className="px-5 mb-6 flex justify-between items-end mt-4">
          <div>
            <h2 className="text-2xl sm:text-[26px] font-bold text-white leading-none capitalize">{activeFilter}</h2>
          ...
          <div className="flex gap-4 items-center">
            <button 
              ...
              className={`p-1 active:scale-90 transition-all ${isSortOpen ? 'text-[#9EF07F]' : 'text-zinc-400'}`}
            >
              <ListFilter size={22} />
            </button>
